...
import { deployAddressStoreFixture, deployFeeVaultFixture, deployProtocolContractsFixture, deployStakePoolFixture, deployStakedBnbTokenFixture, deployTimelockedAdminFixture, deployTokenHubFixture, deployUndelegationHolderFixture } from "./helpers/fixtures";
...
describe("Set-up & Deployment tests", () => {
  ...
  it("should deploy TimelockedAdmin", async () => {
    const { timelockedAdmin, timelockedAdminArgs } = await loadFixture(deployTimelockedAdminFixture);
  ...
  it("should store the addresses", async () => {
    ...
    await expect(storeAddresses(contracts.addressStore, storedContractAddresses)).to.not.be.reverted;
    ...
    expect(await addressStore.getTimelockedAdmin()).to.be.equal(storedContractAddresses.timelockedAdmin);
    expect(await addressStore.getUndelegationHolder()).to.be.equal(storedContractAddresses.undelegationHolder);
  });
});