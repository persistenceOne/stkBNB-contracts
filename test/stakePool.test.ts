...
import {
    createClaimFixture,
    createClaimV1Fixture,
    deployProtocolContractsV1Fixture,
...
describe('Stakepool tests', function () {
    ...
    describe('deposit tests', () => {
        it('should make a deposit', async () => {
            ...
            expect(await stakePoolConnectedUser.deposit({ value: ethers.constants.WeiPerEther })).to
                .not.be.reverted;
        });
        ...
        it('should mint stkBNB correctly when the rewards are distributed', async () => {
            const allContractsDeployment = await loadFixture(setupProtocolFixture);
            const { stakePool, stakedBNBToken, undelegationHolder } =
                allContractsDeployment.contracts;
            ...
            await expect(stakePoolConnectedUser.deposit({ value: ethers.constants.WeiPerEther }))
                .not.to.be.reverted;
    ...
    describe('tokensReceived tests', () => {
        ...
        it('should revert when minting tokens to self - sender address zero', async () => {
            ...
            await expect(
                stkBnBConnectedUser.mint(stakePool.address, ethers.constants.WeiPerEther, [], []),
    ...
    describe('Withdraw tests', () => {
        ...
        it('should create a claim request', async () => {
            ...
            expect(await stakedBNBTokenConnectedUser.send(stakePool.address, stkBnbBalance, [])).to
                .not.be.reverted;
            expect(await stakePool.bnbToUnbond()).to.be.equal(stkBnbBalance);
            expect(await stakePool.getClaimRequestCount(user.address)).to.be.equal(
    ...
    describe('V2 claim features', () => {
        ...
        describe('Instant Claim', () => {
            ...
            beforeEach(async () => {
                allContractsDeployment = await loadFixture(depositBnbFixture);
                user = allContractsDeployment.accounts.user;
                stakedBNBTokenConnectedUser =
                    allContractsDeployment.contracts.stakedBNBToken.connect(user) as StakedBNBToken;
                stakePool = allContractsDeployment.contracts.stakePool as StakePool;
            });

            it('should instantClaim successfully', async () => {
                await expect(
                    stakedBNBTokenConnectedUser.send(
                        stakePool.address,
                        ethers.constants.WeiPerEther,
                        Buffer.from('instant'),
                    ),
            ...
            it('should update the balances correctly when using instantClaim', async () => {
                ...
                const expectedValue = ethers.constants.WeiPerEther.mul(100 - feePercentage).div(
                    100,
                );
        ...
        describe('Automated Claim', () => {
            ...
            beforeEach(async () => {
                ...
                signature = await user._signTypedData(
                    allContractsDeployment.domains.StakePoolDomain,
                    ClaimDataType,
                    claim,
                );
            ...
            it('should make an automatedClaim successfully', async () => {
                await expect(stakePool.automatedClaim(signature, latestClaimIndex, user.address)).to
                    .not.be.reverted;
            });

            it('should update the balances correctly', async () => {
                const expectedValue = ethers.constants.WeiPerEther.sub(
                    STAKE_POOL_CONFIG_V2.automatedClaimFee,
                );
                await expect(
                    stakePool.automatedClaim(signature, latestClaimIndex, user.address),
                ).to.changeEtherBalances([user, stakePool], [expectedValue, expectedValue.mul(-1)]);
            });
        });
    });
