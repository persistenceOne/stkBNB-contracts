...
export async function generateOwnershipTests(data: OwnershipTest) {
    const ownableErrorMessage = data.ownableErrorMessage
        ? data.ownableErrorMessage
        : OWNABLE_ERROR_MESSAGE;
    ...
    describe(`Ownable: ${data.contractObjectName}`, () => {
        ...
        it("Should fail to renounce the ownership if the caller isn't the owner", async () => {
            ...
            await expect(ownableConnectedNonOwner.renounceOwnership()).to.be.revertedWith(
                ownableErrorMessage,
            );
        });
        ...
        it('Should transfer the ownership', async () => {
            ...
            expect(await ownableConnectedOwner.transferOwnership(nonOwner.address)).not.to.be
                .reverted;
        ...
        it("Should fail to transfer the ownership if the caller isn't the owner", async () => {
            ...
            await expect(
                ownableConnectedNonOwner.transferOwnership(nonOwner.address),
