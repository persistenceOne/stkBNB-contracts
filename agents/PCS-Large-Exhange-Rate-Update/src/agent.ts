...
import { SWAP_EVENT, ER_THRESHOLD, PANCAKE_FACTORY_ADDRESS, INIT_CODE_PAIR_HASH, STKBNB_POOL_ADDRESS ,STKBNB_TOKEN_ADDRESS, WBNB_TOKEN_ADDRESS } from "./constants";
    ...
    await Promise.all(
      swapEvents.map(async (event) => {
        const pairAddress = event.address;
          ...
          if(Math.abs((Number(exchangerateOld)-(Number(exchangerateNew))/(Number(exchangerateOld)))*(100))>=(erThreshold)){
